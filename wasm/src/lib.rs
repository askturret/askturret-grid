...
pub fn bench_store_load(count: u32) -> f64 {
    ...
    let start = Date::now();
    let mut store = GridStore::new(&schema.into()).expect("Failed to create GridStore");
    store.load_rows(&rows.into()).expect("Failed to load rows");
    Date::now() - start
}
...
pub fn bench_store_update(count: u32, update_count: u32) -> f64 {
    ...
    let start = Date::now();
    store.batch_update(&updates.into()).expect("Failed to batch update");
    Date::now() - start
}
